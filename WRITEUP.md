...
The repeatable judging scenario is a **historically grounded, simulated 15-minute EWL replay**, not an observed live incident. Its affected eastern corridor and Downtown Line alternative are based on the real [7–10 December 2024 EWL service adjustment between Tampines and Tanah Merah](https://www.lta.gov.sg/content/ltagov/en/newsroom/2024/12/news-releases/reminder_on_train_service_adjustments_along_EWL.html), for which LTA and SMRT advised affected commuters—including those travelling from Tampines towards Raffles Place—to consider the Downtown Line and allow additional travel time. The fixture normalises the impact to 15 minutes to demonstrate Rachel's deadline threshold consistently; it is not an exact reconstruction of the official event. The app labels the scenario and its data source. A five-minute delay and a normal day provide comparison cases. The [README](README.md) gives the setup commands, [five-minute mobile demo recording](https://youtu.be/Ohc0VVrJZ-o), and exact screens to open.
...
- Geographic map tiles require a network connection. The route overlay and an explanatory state remain when tiles cannot load. The mobile flow has been opened and exercised in a real phone browser on the same network, not only in desktop-device emulation. The documented dependency installation, database seed and frontend production build were also exercised from an isolated clean source checkout on Windows.
